...
function Event({ event, drag, config }: { event: IEvent; drag?: IDrag; config?: ICalendarConfig }) {
  ...
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>, end: number) => {
    if (!drag) return;
    e.preventDefault();
    drag.updateEnd(event.date.getDate(), event.date.getMonth() + 1, event.date.getFullYear(), end);
  };
  ...
  return (
    <div
      ...
      className={`cursor-pointer absolute top-[-1px] rounded-lg overflow-hidden border border-white ${drag ? 'z-[89]' : 'z-[90] pointer-events-none'} ${drag?.dragEvent.current == event.id && drag ? 'opacity-50' : ''}`}
    ...
    >
      <div
        ...
        onDragStart={() => drag && drag.startEventDraggin(event.start, event.id)}
      >
      ...
      {Array.from({ length: eventSize - 1 }, (_, index) => (
        <div
          ...
          onDragStart={() => drag && drag.startEventDraggin(event.start + index, event.id)}
          className="flex items-center h-[25px]"
          key={index}
          draggable
        >
